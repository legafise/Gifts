...
public enum MJCCertificatesHandler {
    ...
    MJCCertificatesHandler(String sortName) {
    ...
    public static MJCCertificatesHandler findHandlerByName(String handlerTypeName) {
        return Arrays.stream(values())
                .filter(certificatesSortHandler -> certificatesSortHandler.getHandlerName()
                        .equalsIgnoreCase(handlerTypeName.toUpperCase()))
                .findFirst()
                .orElseThrow(() -> new MJCInvalidSortParameterException(INVALID_HANDLER_MESSAGE));
    }
    ...
    private static List<Certificate> defineListOrder(List<Certificate> certificateList, String orderParameter) {
        ...
        if (!parameterInUpperCase.equals(ASC_PARAMETER) && !parameterInUpperCase.equals(DESC_PARAMETER)) {
            throw new MJCInvalidSortParameterException(INVALID_SORT_PARAMETER_MESSAGE);
        }
