...
public class MJCCertificateController {
    ...
    private EntityModel<CertificateDto> createHateoasCertificateModel(CertificateDto certificate) {
        ...
        certificate.getTags().forEach(tag -> {
            WebMvcLinkBuilder linkToTag = WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(MJCTagController.class)
                    .readTag(tag.getId()));
